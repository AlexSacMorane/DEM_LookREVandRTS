...
def Write_txt(dict_algorithm,dict_sample):
    ...
    for grain in dict_sample['L_g']:
        ...
        if grain.dissolvable :
            file_to_write.write('\tDissolvable : True\n')
        else:
            file_to_write.write('\tDissolvable : False\n')
    ...
    for contact in dict_sample['L_contact']:
        ...
        file_to_write.write('\tNormal : ['+str(round(contact.n12[0],2))+', '+str(round(contact.n12[1],2))+']\n')
        file_to_write.write('\tNormal overlap : '+str(round(contact.overlap,2))+'\n')
        file_to_write.write('\tNormal reaction : '+str(int(contact.F12_n))+'\n')
        file_to_write.write('\tNormal damping : '+str(int(contact.F12_damp_n))+'\n')
        file_to_write.write('\tTangential : ['+str(round(contact.t12[0],2))+', '+str(round(contact.t12[1],2))+']\n')
        ...
        file_to_write.write('\tTangential damping : '+str(int(contact.f12_damp_t))+'\n')
