...
class Grain_pp:
    ...
    def __init__(self, Id, Dissolvable, Center, Coordinate_x, Coordinate_y):
...
def Dissolution_Distribution(dict_sample,dict_sollicitations,simulation_report):
    ...
    while i < int(dict_sollicitations['frac_dissolved']*len(dict_sample['L_g'])):
        ...
        while grain.dissolvable:
            grain = random.choice(dict_sample['L_g'])
        grain.dissolvable = True
        i = i + 1
...
def Plot_chain_force(i_PF,i_DEM):
    ...
    for line in lines :
        ...
        if line == '<grain_c>\n':
            Read_one_grain = False
            L_g.append(Grain_pp(id,dissolvable,center,coordinate_x,coordinate_y))
        elif line == '<contact_c>\n':
            Read_one_contact = False
            ...
            if line[:len('\tid : ')] == '\tid : ':
                ...
                for c in range(len('\tid : ')-1,len(line)):
                    if (line[c]!=' ') and not Read_data:
                        ...
                        Read_data = False
            elif line[:len('\tDissolvable : ')] == '\tDissolvable : ':
                Read_data = False
                for c in range(len('\tDissolvable : ')-1,len(line)):
                    if (line[c]!=' ') and not Read_data:
                        c_start = c
                        Read_data = True
                    elif (line[c]==' ' or c==len(line)-1) and Read_data:
